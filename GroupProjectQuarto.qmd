...
#### Predictive checks
In order to measure the fit of our model to the dataset we observe from Gapminder. If there were a simple lineaer relation between average life expectancy and CO2 emissions we would assume that we could reproduce our observed data by predicting values from a normal fitted function and adding in normally distributed errors. Thus, we simulated a random normal distribution of data with the same standard deviation that we observed from our data set and then compared our observed data to our simulated data using two side by side scatterplots
...
We created these side-by-side comparison using the [patchwork package](https://github.com/thomasp85/patchwork). 
