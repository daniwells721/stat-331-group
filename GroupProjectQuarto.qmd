...
co2_life_twodecade %>%
  ggplot(aes(x = emissions, y = life_exp, color = twodecade)) + # Color by twodecade
  geom_point(size = 0.8, alpha = 0.5) + # Points with slightly reduced opacity for clarity
  geom_smooth(method = "lm", se = FALSE, aes(color = twodecade)) + # Add linear model lines per twodecade
  facet_wrap(~factor(twodecade)) + # Separate plots for each twodecade
  theme_minimal() + # Using a minimal theme for cleaner visuals
  labs(
    title = "Scatterplot of CO2 Emissions vs. Life Expectancy over Time",
    subtitle = "Analyzed across different two-decade periods",
...
```
There is a lot of variability in the actual average life expectancy values, which had a value of about 206.51. Additionally, the high variance of the fitted values suggest that the predictions are also much more spread out. Lastly, the high variance of the residuals means that the residuals are largely spread out, and the relationship is likely  not linear. 
