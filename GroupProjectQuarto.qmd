...
obs_reg_avgco2life <- avg_co2_life %>%
  ...
  theme_bw() + 
  labs(
    title = "Observed Life Expectancy over Time",
    subtitle = "Life Expectancy (years)",
    x = "CO2 Emissions (metric tons per capita)",
    y = ""
  ) +
...
sim_reg_avgco2life <- sim_data %>%
  ggplot(aes(x = avg_emissions, y = sim_life_exp, color = twodecade)) + 
  geom_point(size = 0.8, alpha = 0.5) + 
  geom_smooth(method = "lm", se = FALSE, aes(color = twodecade)) + 
  facet_wrap(~factor(twodecade)) + 
  theme_bw() + 
  labs(
    title = "Simulated Life Expectancy over Time \nbased on Regression Model",
    subtitle = "Life Expectancy (years)",
    x = "CO2 Emissions (metric tons per capita)",
    y = "",
    caption = "Data grouped by two-decade periods to illustrate changes over time."
