...
In order to get a better idea of how the relationship life expectancy and CO2 emissions by country, we took the average of both variables for each country and created a scatterplot. It was then fitted with a line to show the linear regression model.

Once we had the orignal data graphed and had created a linear regression model, we used the model to do predictive checks to see if it was a good fit.
...
From this regression we got the equation: predicted $AvgLifeExpectancy = 56.41 + 0.01(AvgEmissions)$
